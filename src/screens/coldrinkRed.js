...
import { AsyncStorage, FlatList, Image, ScrollView,TextInput, StyleSheet, Text, TouchableOpacity, View, TouchableNativeFeedbackBase } from 'react-native';
...
export default class coldrinkRed extends Component {
    ...
    saveProfile = (namep) => {
        ...
        if(time !== ''){
          database.ref('users').child(this.state.userid).child('time').set(time);
        }
        if(name !== ''){
          database.ref('users').child(this.state.userid).child('name').set(name);
        }
        if(price !== ''){
          database.ref('users').child(this.state.userid).child('price').set(price);
        }
      }

    /*editProduct  = (nameProd) => {
        var id;
        const data = this.state.dataFood
